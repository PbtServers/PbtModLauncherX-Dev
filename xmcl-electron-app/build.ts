...
async function start() {
  ...
  const config: Configuration = {
    ...
    async afterPack(context) {
      ...
      if (!existsSync(src)) {
        src = join(context.appOutDir, 'PbtModLauncherX.app/Contents/Resources/app.asar')
    ...
    async artifactBuildCompleted(context) {
      ...
      if (context.target && context.target.name === 'appx') {
        await buildAppInstaller(version, path.join(__dirname, './build/output/pbtmodlauncherx.appinstaller'), electronBuilderConfig.appx!.publisher!)
      }
    },
  }
